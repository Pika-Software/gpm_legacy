...
do
	...
	function isValidNumber(num, name) -- number must be a positive and integer
		assert( num >= 0, name .. 'must be a valid positive number' )
		assert( math_floor( num ) == num, name .. 'must be a integer' )
	end
end
...
local function parsePreleaseAndBuildmeta(prerelease_and_buildmeta)
	if not prerelease_and_buildmeta or (prerelease_and_buildmeta == '') then
		return
	end
...
local parseVersion, compareIds
do

	local tonumber = tonumber

	function parseVersion( ver )
		local major, minor, patch, prerelease_and_buildmeta = ver:match('^(%d+)%.?(%d*)%.?(%d*)(.-)$')
	...
	function compareIds( id, otherId )
...
local function smallerPrerelease( first, seconds )
	if (first == seconds) or not first then
		return false
